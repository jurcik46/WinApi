...
namespace WinApi
{
    ...
    .CreateLogger();*/
            bool result = false;
            var SignProgram = @"c:\program Files (x86)\adobe\acrobat Reader DC\reader\AcroRd32";
            var file = @"c:\users\jurco\desktop\test.pdf";
            var SignTimeout = 100;
           ...
           /* using (var process = Process.Start(startInfo))
            {
                ...
                do
                {
                    ...
                    var found = FindWindowAndClose(file, false);
...
/*
        private bool FindWindowAndClose(string file, bool close)
        {
            //file = Path.GetFileName(file);
            var caption = string.Format(textBoxCaptionFormat.Text, file);// + " - Visual Studio Code";//string.Format(SettingsService.SignWindowCaptionFormat, file);
